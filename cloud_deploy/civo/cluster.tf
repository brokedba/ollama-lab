locals {
  node_size = coalesce(var.cluster_node_size, element(data.civo_size.ai.sizes, 0).name)
  kubernetes_version = (
...
resource "civo_kubernetes_cluster" "cluster" {
  ...
  kubernetes_version  = "talos-v1.5.0" #local.kubernetes_version
...
resource "kubernetes_namespace" "webui_ns" {
  depends_on = [local_file.cluster-config]
  metadata {
    name = "open-webui"
  }
